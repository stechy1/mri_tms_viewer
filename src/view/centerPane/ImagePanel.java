					...
					try {
					    ...
					    for (GroupModel	group : model.getGroups()) {
					    	if(group.getPoints().size() != 0){
					    		...
					    		ArrayList<MyPoint> pointsToDraw = group.getPointFromLayer(model.getActualSnapshot());
								...
								if(pointsToDraw.size() >=3){
									if(! (group.getName().equals(Configuration.UNASSIGN_GROUP) || group.getName().equals(Configuration.IGNORE_GROUP))){
										...
										ArrayList<MyPoint> hull = new QuickHull().quickHull(pointsToDraw);
										for (int i = 0; i < hull.size(); i++) {
											g2.setStroke(new BasicStroke(3));
											g2.drawLine((int) (hull.get(i).getCenterX() * ratio + this.x_offset), 
														(int) (hull.get(i).getCenterY() * ratio + this.y_offset), 
														(int) (hull.get((i+1)%hull.size()).getCenterX() * ratio + x_offset), 
														(int) (hull.get((i+1)%hull.size()).getCenterY() * ratio + y_offset));
										}
										
										double area = 0.0;
										
										String pixelSpace = getTagValue(dcm, DicomTags.PIXEL_SPACING);
										pixelSpace = pixelSpace.substring(0, pixelSpace.indexOf("\\"));
