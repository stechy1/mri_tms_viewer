...
public class ImagePanelModel {
    ...
    public void createGroups(int count) {
        ...
        for (int i = 0; i < groups.size(); i++) {
            ...
            if (!(groups.get(i).getName().equals(Configuration.IGNORE_GROUP) || groups.get(i)
                .getName().equals(Configuration.UNASSIGN_GROUP))) {
                groups.remove(i);
                i--;
            }
        }
    ...
    private void assignPointToGroups(ArrayList<MyPoint> list) {
        ...
        while (countOfChanges != 0) {
            ...
            for (MyPoint myPoint : list) {
                ...
                for (GroupModel group : groups) {
                    if (!(group.getName().equals(Configuration.UNASSIGN_GROUP) || group.getName()
                        .equals(Configuration.IGNORE_GROUP))) {
